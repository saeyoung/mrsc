#############################################################
#
# Test 7. Multinomial Process Test
#
#############################################################
import sys, os
sys.path.append("../..")
...
def experiment_2(powers):
    ...
    for n in [2,3,5,8,13,21]:
        ...
        plt.savefig("result/boxplot/multinomial/{}_states_multinomial_boxplot_semilog_{}_samples_{}.png".format(n, samples, powers[0]))
        # plt.show()
        plt.clf()
        ...
        plt.xticks(np.arange(1,len(powers)+1),powers)
        plt.savefig("result/boxplot/multinomial/{}_states_multinomial_boxplot_loglog_{}_samples_{}.png".format(n, samples,powers[0]))
        plt.plot(np.arange(1,len(powers)+1),reg.predict(X), color="red", label="r2 score={}".format(reg.score(X,y).round(3)))
        plt.legend()
        plt.savefig("result/boxplot/multinomial/{}_states_multinomial_boxplot_loglog_{}_samples_regression_{}.png".format(n, samples,powers[0]))
        # plt.show()
        plt.clf()
